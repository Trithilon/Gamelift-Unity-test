  ...
  public abstract class Api {
    ...
    private async UniTask AddRequestBody(IRequest request, object body) {
      if (body != null) {
        ...
        request.UploadHandler = new UploadHandlerRaw(bodyString.GetBytes()) {contentType = "application/json"};
      }
    }
    ...
    public async UniTask<Response> SendRequest(RequestOptions requestOptions, IRequest request) {
      using (request.UnityWebRequest) {
        ...
        throw new RequestException(response.Error, response.IsHttpError, response.IsNetworkError, response.StatusCode,
          response.Text);
      }
    }

    private async UniTask<Response> Request(RequestOptions requestOptions) =>
      await SendRequest(requestOptions, new Request());
